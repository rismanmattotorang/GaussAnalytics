...
| Toucan2 ORM + entities | App metadata persistence | `gauss-db` repositories | `sqlx` (P2) | 1→2 | ◐ |
| Liquibase migrations | Schema versioning | SQL migrations + `gaussctl migrate` | `sqlx::migrate` (P2) | 2 | ☐ |
| JDBC data-source drivers | Connect to warehouses | `gauss-drivers` (under `gauss-query`) | `sqlx`, native connectors | 2 | ☐ |
| Auth / sessions | Login, sessions, API keys | `gauss-auth` | `argon2` | 1→2 | ◐ |
| Permissions / RBAC | Collection/DB-scoped access | `gauss-auth::perms` | (std) | 1→2 | ◐ |
| Database sync / fingerprint / scan | Discover schema + stats | `gauss-sync` (under `gauss-db`) | `sqlx`, `tokio` | 2 | ☐ |
